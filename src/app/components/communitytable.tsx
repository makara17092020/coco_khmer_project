...
function CommunityImage({ image, alt }: { image: string; alt: string }) {
...
export default function CommunityTable() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredCommunity.length / ITEMS_PER_PAGE)
  );
  ...
  const handleCreateImage = () => router.push("/admin/community/create");
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                paginatedCommunity.map(({ id, createdAt, image, name }) => (
                  <tr key={id} className="hover:bg-green-50 transition">
                    <td className="px-4 py-4 whitespace-nowrap">
                      <CommunityImage image={image} alt={name || "No name"} />
                    </td>
